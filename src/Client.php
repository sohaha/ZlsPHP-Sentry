...
class Raven_Client
{
    ...
    public function __construct($options_or_dsn = null, $options = [])
    {
        ...
        $this->tags                    = Raven_Util::get($options, 'tags', []);
        ...
        $this->exclude                 = Raven_Util::get($options, 'exclude', []);
        $this->excluded_exceptions     = Raven_Util::get($options, 'excluded_exceptions', []);
        $this->severity_map            = null;
        $this->http_proxy              = Raven_Util::get($options, 'http_proxy');
        $this->extra_data              = Raven_Util::get($options, 'extra', []);
        ...
        $this->sdk                             = Raven_Util::get($options, 'sdk', [
            'name'    => 'zls-sentry',
    ...
    {
        return 'zls-sentry/' . self::getVersion();
    }
    ...
    public static function getVersion()
    {
        ...
        if (!$version) {
            ...
            $composerLock = json_decode(@file_get_contents(z::realPath('composer.lock', false, false)), true);
            $version      = z::tap($composerLock,
                static function (&$composer) {
                    ...
                    $composer = z::arrayFilter(z::arrayValues($composer, ['name', 'version']), function ($v) {
                        return $v['name'] === 'zls/sentry';
                    });
                    $composer = z::arrayMap($composer, function ($v) {
                        $v['version'] = z::strBeginsWith($v['version'], 'v') ? substr($v['version'], 1) : $v['version'];

                        return $v;
                    });
                    $composer = Z::arrayGet(array_values($composer), '0.version', '0.0.0');
                });
        }
    ...
    public function captureException($exception, $data = null, $logger = null, $vars = null)
    {
        ...
        do {
             ...
             */
            $trace = $exc->getTrace();
            if ($exc instanceof \Zls_Exception) {
    ...
    public function capture($data, $stack = null, $vars = null)
    {
        ...
        $runtime_context             = ['version' => PHP_VERSION, 'name' => 'php'];
    ...
    public function send(&$data)
    {
        ...
        $headers = [
            ...
            'X-Sentry-Token' => 'a29d5a50c30c11e98ed20242ac130010',
    ...
    protected function get_curl_options()
    {
        $options = [
            ...
            CURLOPT_USERAGENT      => 'zls-sentry/' . self::getVersion(),
        ];
        if ($this->http_proxy) {
